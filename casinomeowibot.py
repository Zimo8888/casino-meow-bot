...
# تنظیمات ربات از دیتابیس خوانده می‌شوند تا بعد از ری‌استارت Railway از بین نروند.
def load_bot_settings():
    with db_lock:
        rows = conn.cursor().execute("SELECT key, value FROM bot_settings").fetchall()
    for key, value in rows:
        if key in bot_settings:
            if key == 'active_card_id':
                bot_settings[key] = value
            else:
                bot_settings[key] = value == '1'
...
# ================= بکاپ و بازگردانی دائمی =================
pending_restore = set()


def create_backup_file(created_by):
    timestamp = time.strftime('%Y-%m-%d_%H-%M-%S')
    filename = f'bot_data_{timestamp}_{created_by}.db'
    ...
    with db_lock:
        ...
        conn.cursor().execute(
            "INSERT INTO backup_logs (filename, created_at, created_by, size_bytes) VALUES (?, datetime('now'), ?, ?)" ,
...
def backup_files_for_admin():
    with db_lock:
        return conn.cursor().execute(
            'SELECT id, filename, created_at, created_by, size_bytes FROM backup_logs ORDER BY id DESC'
        ).fetchall()


@bot.message_handler(commands=['backup'], func=lambda m: m.chat.type == 'private' and m.from_user.id in ADMINS)
def backup_command(message):
    try:
        path = create_backup_file(message.from_user.id)
        with open(path, 'rb') as f:
            bot.send_document(
                message.chat.id,
                f,
                caption=f'💾 بکاپ با موفقیت ساخته شد.\n\n📁 {os.path.basename(path)}\n🗂 این بکاپ روی Volume هم ذخیره شد.'
            )
    except Exception as e:
        bot.send_message(message.chat.id, f'❌ ساخت بکاپ ناموفق بود.\n\n{e}')


@bot.message_handler(commands=['backups'], func=lambda m: m.chat.type == 'private' and m.from_user.id in ADMINS)
def backups_command(message):
    rows = backup_files_for_admin()
    if not rows:
        return bot.send_message(message.chat.id, '📂 هنوز هیچ بکاپی ثبت نشده است.')
    lines = ['🗂 <b>بکاپ‌های ثبت‌شده</b>\n']
    for bid, filename, created_at, created_by, size_bytes in rows[:20]:
        size_kb = max(1, size_bytes // 1024)
        lines.append(f'#{bid} | <code>{html_escape_backup(filename)}</code> | {created_at} | {size_kb} KB')
    bot.send_message(message.chat.id, '\n'.join(lines), parse_mode='HTML')


def html_escape_backup(value):
    return str(value).replace('&', '&amp;').replace('<', '&lt;').replace('>', '&gt;')


@bot.message_handler(commands=['restore'], func=lambda m: m.chat.type == 'private' and m.from_user.id in ADMINS)
def restore_command(message):
    pending_restore.add(message.from_user.id)
    bot.send_message(message.chat.id, '♻️ فایل بکاپ دیتابیس را همینجا ارسال کن.\n\nفقط بکاپی را بفرست که از همین ربات گرفته‌ای.')


@bot.message_handler(content_types=['document'], func=lambda m: m.chat.type == 'private' and m.from_user.id in ADMINS and m.from_user.id in pending_restore)
def restore_document(message):
    temp_path = os.path.join(DB_DIR, 'restore_temp.db')
    try:
        ...
        with db_lock:
            ...
            globals()['conn'] = sqlite3.connect(DB_PATH, check_same_thread=False)
            globals()['conn'].execute('PRAGMA journal_mode=WAL')
            c = globals()['conn'].cursor()
            c.execute('CREATE TABLE IF NOT EXISTS bot_settings (key TEXT PRIMARY KEY, value TEXT)')
            c.execute('''CREATE TABLE IF NOT EXISTS backup_logs (id INTEGER PRIMARY KEY AUTOINCREMENT, filename TEXT NOT NULL, created_at TEXT NOT NULL, created_by INTEGER NOT NULL, size_bytes INTEGER DEFAULT 0)''')
            globals()['conn'].commit()

        load_bot_settings()
        pending_restore.discard(message.from_user.id)
        bot.send_message(message.chat.id, '✅ دیتابیس با موفقیت بازگردانی شد.\n\n♻️ اطلاعات قبلی با فایل بکاپ جایگزین شد.')
    except Exception as e:
        pending_restore.discard(message.from_user.id)
        try:
            ...
            pass
        bot.send_message(message.chat.id, f'❌ بازگردانی انجام نشد.\n\n{e}')
...
def callback_handler(call):
    ...
    if data.startswith("alock_") and user_id in ADMINS:
        ...
        if key in bot_settings:
            ...
            save_bot_setting(key, 1 if bot_settings[key] else 0)
    ...
    # خروجی گرفتن لیست کاربران به صورت فایل txt
        ...
        save_bot_setting('active_card_id', bot_settings['active_card_id'])
